...
def understanding_vs_participant(df):
	...
	def clean_row(row):
		understanding = row['Understanding Metric']
		understanding = discard_non_float_conversion_func(understanding)
		#num_flips = row['numFlipsToCheatSheet']
		#num_new_data = row['numUserNewDataToTool']
		#num_flips = discard_non_int_conversion_func(num_flips)
		#num_new_data = discard_non_int_conversion_func(num_new_data)
		...
		if understanding is None:
			...
			return (understanding) #num_new_data, num_flips)

	# CREATE DICTIONARY, FILL WITH DATA BY TOOL/CONTROL
	fil = dict()
	fil['control'] = [('context', re.compile(r"control."))]
	fil['regexr'] = [('context', re.compile(r"explain."))]
	fil['regexper'] = [('context', re.compile(r"automata."))]
	...
	# CREATE DATA OBJECT POPULATED WITH UNDERSTANDING METRIC FOR FOUR SCENARIOS (control, regexr, regexper, grex)
	data = dict()
	for method in methods:
		data[method] = get_data_dict()
		...
		for i, row in enumerate(rows):
			...
			if fixed_row is None:
				...
				data[method]['understanding'].append(fixed_row)

	# AVERAGE DATA FOR UNDERSTANDING
	average_understanding_control = np.average(data['control']['understanding'])
	average_understanding_regexr = np.average(data['regexr']['understanding'])
	average_understanding_regexper = np.average(data['regexper']['understanding'])
	average_understanding_grex = np.average(data['grex']['understanding'])
	print(average_understanding_control, average_understanding_regexr, average_understanding_regexper, average_understanding_grex)

	labels = ['All Participants'] #, 'Less Experienced Participants', 'Experienced Participants']




	# average understanding
	# -> understanding metric -> split by tool -> average
								# -> control
								# -> regexr
								# -> regexper
								# -> grex

	# -> split by tool -> understanding metric
...
def main():
	...
	understanding_vs_participant(df)
